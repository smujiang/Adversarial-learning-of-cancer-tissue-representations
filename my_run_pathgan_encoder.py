...
# parser.add_argument('--check_every', dest='check_every', type=int, default=10,
#                     help='Save checkpoint and generate samples every X epcohs.')
# parser.add_argument('--restore', dest='restore', action='store_true', default=False,
#                     help='Restore previous run and continue.')
...
model = "PathologyGAN_Encoder"  # default
...
epochs = 300
batch_size = 8
# main_path = "/infodev1/non-phi-data/junjiang/Path_GAN/TSR_data_model"
main_path = "/infodev1/non-phi-data/junjiang/Path_GAN/HGSOC_SBOT_model"
dbs_path = "/infodev1/non-phi-data/junjiang/Path_GAN/TSR_data"
restore = True
report = True
check_every = 10
...
with tf.Graph().as_default():
    # Instantiate Model.
    pathgan_encoder = PathologyGAN_Encoder(data=data, z_dim=z_dim, layers=layers, use_bn=use_bn, alpha=alpha,
                                           beta_1=beta_1, init=init, regularizer_scale=regularizer_scale,
                                           ...
                                           learning_rate_d=learning_rate_d, learning_rate_e=learning_rate_e,
                                           beta_2=beta_2, n_critic=n_critic, gp_coeff=gp_coeff,
                                           loss_type=loss_type, model_name=model)
    # Training.
    losses = pathgan_encoder.train(epochs=epochs, data_out_path=data_out_path, data=data, restore=restore,
                                   print_epochs=10, checkpoint_every=check_every, report=report)
