...
label_storage_train = hdf5_file_train.create_dataset(name='train_labels', shape=[train_cnt, 3], dtype=np.uint8)
img_storage_val = hdf5_file_val.create_dataset(name='validate_img', shape=[val_cnt, 224, 224, 3], dtype=int)
label_storage_val = hdf5_file_val.create_dataset(name='validate_labels', shape=[val_cnt, 3], dtype=np.uint8)
img_storage_test = hdf5_file_test.create_dataset(name='test_img', shape=[test_cnt, 224, 224, 3], dtype=int)
label_storage_test = hdf5_file_test.create_dataset(name='test_labels', shape=[test_cnt, 3], dtype=np.uint8)
train_pt = 0
val_pt = 0
test_pt = 0
for c in case_list:
    ...
    labels = np.array(hdf5_file["tsr_scores"])
