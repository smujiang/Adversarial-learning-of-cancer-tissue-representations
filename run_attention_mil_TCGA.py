...
parser.add_argument('--att_dim', dest='att_dim', type=int, default=5,
                    help='Dimensions of the attention network, default is 150.')
parser.add_argument('--bag_size', dest='bag_size', type=int, default=10000,
                    help='Maximum number of instaces for a bag, default is 10K.')
...
parser.add_argument('--gan_model', dest='gan_model', type=str, default='PathologyGAN_Encoder',
                    help='Model name of latent projections.')
parser.add_argument('--hdf5_file_path', dest='hdf5_file_path', type=str, default=None, required=True,
                    help='Path for latent representations H5 file, results on loss.csv.')
parser.add_argument('--hdf5_file_path_add', dest='hdf5_file_path_add', type=str, default=None, required=False,
                    help='Path for additional latent representations H5 file, results on loss_2.csv.')
parser.add_argument('--gated', dest='gated', action='store_true', default=False,
                    help='Gated architecture for attention.')
...
image_channels = 3
name_run = 'h%s_w%s_n%s_zdim%s_att%s_hlatent_%s_gated_%s' % (
image_height, image_width, image_channels, z_dim, att_dim, h_latent, gated)
...
with tf.Graph().as_default():
    ...
    amil.train(epochs=epochs, hdf5_file_path=hdf5_file_path, data_out_path=data_out_path, folds=folds,
               hdf5_file_path_add=hdf5_file_path_add, h_latent=h_latent)
