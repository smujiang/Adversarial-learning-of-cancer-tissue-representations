...
def generate_samples(model, n_images, data_out_path, name='geneated_samples.png'):
    ...
    with tf.Session() as session:
        ...
        # Sample images.
        gen_samples, sample_z = show_generated(session=session, z_input=model.z_input, z_dim=model.z_dim,
                                               output_fake=model.output_gen, n_images=n_images, show=False)
...
def linear_interpolation(model, n_images, data_out_path, orig_vector, dest_vector):
    ...
    with tf.Session() as session:
        ...
        # feed_dict = {model.z_input_1: sequence}
        # feed_dict = {model.z_input_2: sequence}
        w_latent_in = np.tile(sequence[:, :, np.newaxis], [1, 1, model.layers + 1])
