...
def mapping_resnet(z_input, z_dim, layers, reuse, is_train, spectral, activation, normalization, init='xavier',
                   regularizer=None, name='mapping_network'):
    if display:
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        for layer in range(layers):
            net = residual_block_dense(inputs=net, is_training=is_train, normalization=normalization, use_bias=True,
...
def mapping_rescale(w_input, w_dim, layers, cond_label, reuse, is_train, spectral, activation, normalization,
                    init='xavier', regularizer=None, name='mapping_rescale'):
    if display:
        print('MAPPING NETWORK INFORMATION:', name)
        print('Layers:      ', layers)
        print('Normalization: ', normalization)
    ...
    with tf.variable_scope(name, reuse=reuse):
        net = w_input
        for layer in range(layers):
            net = residual_block_dense(inputs=net, is_training=is_train, normalization=normalization,
                                       cond_label=cond_label, use_bias=True, spectral=spectral, activation=activation,
...
# TODO: create a new function, similar to "generator_resnet_style", but with weight disentangle
# use "residual_block_disentangle" to replace "residual_block"
# "convolution" may also need to be replaced by "decomposition_conv2d_layer"
def generator_resnet_style(w_input, image_channels, layers, spectral, activation, reuse, is_train, normalization,
                           init='xavier', noise_input_f=False, regularizer=None, cond_label=None,
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        # Dense.
        net = dense(inputs=net, out_dim=256 * i_pixel * i_pixel, spectral=spectral, init=init, regularizer=regularizer,
                    scope=2)
        ...
        for layer in range(layers):
            ...
            net = residual_block(inputs=net, filter_size=3, stride=1, padding='SAME', scope=layer, is_training=is_train,
                                 spectral=spectral, init=init, regularizer=regularizer, noise_input_f=noise_input_f,
            ...
            net = convolutional(inputs=net, output_channels=reversed_channel[layer], filter_size=2, stride=2,
                                padding='SAME', conv_type=up, spectral=spectral, init=init, regularizer=regularizer,
                                scope=layer)
        ...
        if stack_layers:
            print('Adding layer output to stack layer output.')
            out_stack_layers.append(net)
        logits = convolutional(inputs=net, output_channels=image_channels, filter_size=3, stride=1, padding='SAME',
                               conv_type='convolutional', spectral=spectral, init=init, regularizer=regularizer,
                               scope='logits')
...
def generator_resnet_style_modulation(w_input, image_channels, layers, spectral, activation, reuse, is_train,
                                      normalization, init='xavier', noise_input_f=False, regularizer=None,
                                      cond_label=None, attention=None, up='upscale', name='generator'):
    channels = [32, 64, 128, 256, 512, 1024]
    reversed_channel = list(reversed(channels[:layers]))
    i_pixel = 7
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        # Dense.
        net = dense(inputs=w_input_block, out_dim=1024, spectral=spectral, init=init, regularizer=regularizer, scope=1)
        if normalization is not None: net = normalization(inputs=net, training=is_train, c=label, spectral=spectral,
                                                          scope='dense_1')
        net = activation(net)
        ...
        for layer in range(layers):
            ...
            label = w_input[:, :, layer]
            # ResBlock.
            net = residual_block_mod(inputs=net, filter_size=3, stride=1, padding='SAME', scope=layer,
                                     is_training=is_train, spectral=spectral, init=init, regularizer=regularizer,
                                     noise_input_f=noise_input_f, activation=activation, normalization=normalization,
                                     cond_label=label)
...
def generator_msg(w_input, image_channels, layers, spectral, activation, reuse, is_train, normalization, init='xavier',
                  noise_input_f=False, regularizer=None, cond_label=None, attention=None, up='upscale'):
    ...
    with tf.variable_scope('generator', reuse=reuse):
        ...
        for layer in range(layers):
            ...
            if net.shape.as_list()[1] >= 64:
                msg_i = convolutional(inputs=net, output_channels=image_channels, filter_size=1, stride=1,
                                      padding='SAME', conv_type='convolutional', spectral=spectral, init=init,
                                      regularizer=regularizer, scope='msg_%s' % layer)
        ...
        net = convolutional(inputs=net, output_channels=image_channels, filter_size=3, stride=1, padding='SAME',
                            conv_type='convolutional', spectral=spectral, init=init, regularizer=regularizer,
                            scope='conv_logits')
        logits = residual_block(inputs=net, filter_size=3, stride=1, padding='SAME', scope='resnet_logits',
                                is_training=is_train, spectral=spectral, init=init, regularizer=regularizer,
                                noise_input_f=noise_input_f, activation=activation, normalization=normalization,
                                cond_label=label)
        logits = convolutional(inputs=net, output_channels=image_channels, filter_size=3, stride=1, padding='SAME',
...
def generator_resnet(z_input, image_channels, layers, spectral, activation, reuse, is_train, normalization,
                     init='xavier', noise_input_f=False, regularizer=None, cond_label=None, attention=None,
                     up='upscale', bigGAN=False, name='generator'):
    channels = [32, 64, 128, 256, 512, 1024]
    reversed_channel = list(reversed(channels[:layers]))
