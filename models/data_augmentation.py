...
def random_apply(func, p, x):
    return tf.cond(tf.less(tf.random_uniform([], minval=0, maxval=1, dtype=tf.float32), tf.cast(p, tf.float32)),
...
def distorted_bounding_box_crop(image, bbox, min_object_covered=0.1, aspect_ratio_range=(0.75, 1.33),
    ...
    with tf.name_scope(scope, 'distorted_bounding_box_crop', [image, bbox]):
        ...
        sample_distorted_bounding_box = tf.image.sample_distorted_bounding_box(shape, bounding_boxes=bbox,
...
def crop_and_resize(image, height, width, area_range):
    ...
    image = distorted_bounding_box_crop(image, bbox, min_object_covered=0.1,
                                        aspect_ratio_range=(3. / 4 * aspect_ratio, 4. / 3. * aspect_ratio),
                                        area_range=area_range, max_attempts=100, scope=None)
...
def random_apply_sobel(func, p, x):
    return tf.cond(tf.less(tf.random_uniform([], minval=0, maxval=1, dtype=tf.float32), tf.cast(p, tf.float32)),
                   lambda: tf.reduce_mean(func(x), axis=-1), lambda: x)
...
############### DATA AUG WRAPPER ############### 
def data_augmentation(images, crop, rotation, flip, g_blur, g_noise, color_distort, sobel_filter, img_size,
                      num_channels):
    images_trans = images
