...
class Attention_MIL():
    ...
    def feature_extractor(self, inputs, use):
        interm = inputs
        if use:
            net = dense(inputs=inputs, out_dim=int(self.z_dim / 2), scope=1, use_bias=True, spectral=False,
                        init='xavier', regularizer=None, display=True)
            # net = ReLU(net)
    ...
    def attention(self, inputs):
        net1 = dense(inputs=inputs, out_dim=self.att_dim, scope=3, use_bias=True, spectral=False, init='xavier',
                     regularizer=None, display=True)
        net1 = tanh(net1)
        net2 = dense(inputs=inputs, out_dim=self.att_dim, scope=4, use_bias=True, spectral=False, init='xavier',
                     regularizer=None, display=True)
        net2 = sigmoid(net2)
        net = net1 * net2
        net = dense(inputs=net, out_dim=1, scope=5, use_bias=True, spectral=False, init='xavier', regularizer=None,
                    display=True)
    ...
    def classifier(self, interm, weights):
        ...
        net = dense(inputs=z, out_dim=1, scope=10, use_bias=True, spectral=False, init='xavier', regularizer=None,
                    display=True)
        prob = sigmoid(net)
        return prob, z
    ...
    def optimization(self):
        trainer = tf.train.AdamOptimizer(learning_rate=self.learning_rate, beta1=self.beta_1,
                                         beta2=self.beta_2).minimize(self.loss)
        return trainer
    ...
    # Train function.
    def train(self, epochs, hdf5_file_path, data_out_path, dataset_name='crc_histophen', img_size=56, test_size=9,
              folds=10, label_dim=0):
        ...
        for fold in range(folds):
            ...
            with tf.Session(config=config) as session:
                ...
                for epoch in range(1, epochs + 1):
                    ...
                    update_csv(model=self, file=csvs[0], variables=[np.mean(e_losses)], epoch=epoch,
                ...
                with open(os.path.join(fold_output_path, 'test_results.txt'), 'w') as file_content_test:
                    with open(os.path.join(fold_output_path, 'train_results.txt'), 'w') as file_content_train:
                        ...
                        for sample_indx in sample_indxs:
                            ...
                            if 'crc' in dataset_name:
                                file_img = '/media/adalberto/Disk2/PhD_Workspace/dataset_preprocessing/crc_histophen/Selected_%s/img%s.png' % (
                                img_size, sample_indx)
                                if label_dim == 0:
                                ...
                                file_img_epi = '/media/adalberto/Disk2/PhD_Workspace/dataset_preprocessing/crc_histophen/Selected_%s/img%s_mask_%s.png' % (
                                img_size, sample_indx, cell_type)
                                img_epi = np.array(Image.open(file_img_epi)) / 255.
                            elif dataset_name == 'nki':
                                file_img = '/media/adalberto/Disk2/PhD_Workspace/dataset_preprocessing/nki/images/Selected_%s/%s' % (
                                img_size, sample_indx)
                            elif dataset_name == 'bisque':
                                file_img = '/media/adalberto/Disk2/PhD_Workspace/dataset_preprocessing/bisque/Selected_%s/%s_ccd.png' % (
                                img_size, sample_indx)
                                file_gt = '/media/adalberto/Disk2/PhD_Workspace/dataset_preprocessing/bisque/Selected_%s/%s_ccd_gt.png' % (
                                img_size, sample_indx)
                            ...
                            if 'crc' in dataset_name:
                                ...
                                plt.imsave(os.path.join(imgs_path, 'img%s_mask_%s.png' % (sample_indx, cell_type)),
                                           img_epi)
                            elif dataset_name == 'nki':
                                plt.imsave(os.path.join(imgs_path, str(sample_indx)), img)
                                plt.imsave(os.path.join(imgs_path, sample_indx.replace('.jpg', '_weighted.jpg')),
                                           weighted_img)
                            elif dataset_name == 'bisque':
                                gt = np.array(Image.open(file_gt)) / 255.
                                plt.imsave(os.path.join(imgs_path, '%s_ccd.png' % str(sample_indx)), img)
                                plt.imsave(os.path.join(imgs_path, '%s_ccd_weighted.png' % str(sample_indx)),
                                           weighted_img)
                                plt.imsave(os.path.join(imgs_path, '%s_ccd_gt.png' % str(sample_indx)), gt)

                            file_content.write(' '.join(
                                ['Test sample', str(sample_indx), 'Prob:', str(prob[0]), 'Class:', str(clas[0]),
                                 'Label:', str(label_batch), 'Patches:', str(indxs.shape[0]), '\n']))
