...
def get_projections_all_dataset_projections(model, data, session, run_options, data_out_path, num_clusters,
                                            batch_size=50):
    ...
    with h5py.File(hdf5_features, mode='w') as hdf5_features_file:
        features_storage = hdf5_features_file.create_dataset(name='features',
                                                             shape=(num_samples, model.features_fake.shape[1]),
                                                             dtype=np.float32)
        ...
        if np.unique(model.feature_labels).shape[0] > 1:
            ...
            match = hungarian_matching(model=model, new_classes=new_classes, current_classes=model.feature_labels,
        ...
        feature_labels_storage = hdf5_features_file.create_dataset(name='feat_cluster_labels',
                                                                   shape=[num_samples] + [1], dtype=np.float32)
        embedding_storage = hdf5_features_file.create_dataset(name='embedding', shape=[num_samples] + [2],
                                                              dtype=np.float32)
