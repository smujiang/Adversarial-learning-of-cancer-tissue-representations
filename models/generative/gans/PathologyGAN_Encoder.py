...
class PathologyGAN_Encoder(GAN):
    def __init__(self,
                 ...
                 power_iterations=1,
                 # Iterations of the power iterative method: Calculation of Eigenvalues, Singular Values.
        ...
        # Super Initializer.
        super().__init__(data=data, z_dim=z_dim, use_bn=use_bn, alpha=alpha, beta_1=beta_1,
    ...
    # GAN inputs; override the parent function
    def model_inputs(self):

        # Image input.
        real_images_1 = tf.placeholder(dtype=tf.float32,
                                       shape=(None, self.image_width, self.image_height, self.image_channels),
                                       name='real_images_1')
        real_images_2 = tf.placeholder(dtype=tf.float32,
                                       shape=(None, self.image_width, self.image_height, self.image_channels),
                                       name='real_images_2')
        # Latent vectors.
    ...
    def encoder(self, images, reuse, is_train, init):
        ...
        w_latent = encoder_resnet_instnorm(images=images, latent_dim=self.z_dim, layers=self.layers,
                                           spectral=self.spectral,
                                           activation=leakyReLU, reuse=reuse, is_train=is_train, init=init,
    ...
    def mapping(self, z_input, reuse, is_train, normalization, init):
        w_latent = mapping_resnet(z_input=z_input, z_dim=self.z_dim, layers=self.synth_layers, reuse=reuse,
                                  is_train=is_train, spectral=self.spectral, activation=ReLU,
                                  normalization=normalization, init=init,
                                  regularizer=orthogonal_reg(self.regularizer_scale))
        return w_latent

    # Generator Network. ; override the parent function
    def generator(self, w_latent, reuse, is_train, init):
        output = generator_resnet_style(w_input=w_latent, image_channels=self.image_channels, layers=self.layers,
                                        spectral=self.spectral, activation=leakyReLU, reuse=reuse,
                                        is_train=is_train, normalization=self.normalization, init=init,
                                        noise_input_f=self.noise_input_f, attention=self.attention,
                                        regularizer=orthogonal_reg(self.regularizer_scale))
        return output

    # Discriminator Network. ; override the parent function
    def discriminator(self, images, reuse, init, name, label_input=None):
        output, logits, feature_space = discriminator_resnet(images=images, layers=self.layers, spectral=self.spectral,
                                                             activation=leakyReLU, reuse=reuse,
                                                             attention=self.attention,
                                                             normalization=None, feature_space_flag=True, init=init,
                                                             regularizer=orthogonal_reg(self.regularizer_scale),
                                                             label=label_input,
                                                             name=name)
        return output, logits, feature_space

    # Loss Function. ; override the parent function
    def loss(self):
        loss_dis, loss_gen = losses(self.loss_type, self.output_fake, self.output_real, self.logits_fake,
                                    self.logits_real, real_images=self.real_images_1, fake_images=self.fake_images,
                                    top_k_samples=self.top_k_samples, discriminator=self.discriminator,
                                    gp_coeff=self.gp_coeff, init=self.init, dis_name='discriminator_gen')
    ...
    # Optimizer. ; override the parent function
    def optimization(self):
        train_discriminator, train_generator, _ = optimizer(self.beta_1, self.loss_gen, self.loss_dis, self.loss_type,
                                                            self.learning_rate_input_d,
                                                            self.learning_rate_input_g, None, beta_2=self.beta_2,
                                                            gen_name='generator', dis_name='discriminator_gen',
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            ...
            train_encoder = tf.train.AdamOptimizer(learning_rate=self.learning_rate_input_e,
    ...
    def style_mixing_reg(self, w_input_1, w_input_2, style_mixing_prob, layers):
        ...
        with tf.variable_scope('style_mixing_reg'):
            ...
            layer_cut = tf.cond(tf.random_uniform([], 0.0, 1.0) < style_mixing_prob,
                                lambda: tf.random.uniform([], 1, layers_index, dtype=tf.int32),
    ...
    # Build the GAN. ; override the parent function
    def build_model(self):
        with tf.device('/gpu:0'):
            ...
            ################### TRAINING #####################################################################################################################################
            # Generator Training.
            self.w_latent_1 = self.mapping(self.z_input_1, reuse=False, is_train=True, normalization=None,
                                           init=self.init)
            self.w_latent_2 = self.mapping(self.z_input_2, reuse=True, is_train=True, normalization=None,
                                           init=self.init)
            ...
            self.output_fake, self.logits_fake, self.feature_space_fake = self.discriminator(images=self.fake_images,
                                                                                             reuse=False,
                                                                                             init=self.init,
                                                                                             name='discriminator_gen')
            self.output_real, self.logits_real, self.feature_space_real = self.discriminator(images=self.real_images_1,
                                                                                             reuse=True, init=self.init,
                                                                                             name='discriminator_gen')
            # Encoder Training.
            ...
            ################### INFERENCE #####################################################################################################################################
            # Generator Inference.
            self.w_latent_out = self.mapping(self.z_input_1, reuse=True, is_train=False, normalization=None,
                                             init=self.init)
        ...
        with tf.Session(config=config) as session:
            ...
            for epoch in range(1, epochs + 1):
                ...
                for batch_images, batch_labels in data.training:
                    ...
                    feed_dict = {self.z_input_1: z_batch_1, self.z_input_2: z_batch_2, self.w_latent_in: w_latent_in,
                                 self.real_images_1: batch_images, self.real_images_2: batch_images,
                                 self.style_mixing_prob: self.style_mixing,
                                 self.learning_rate_input_g: self.learning_rate_g,
                                 self.learning_rate_input_d: self.learning_rate_d,
                                 self.learning_rate_input_e: self.learning_rate_e}
                    # Update discriminator.
                    _, gen_img, w_latent_e_1 = session.run([self.train_discriminator, self.fake_images, self.w_latent],
                    ...
                    if flag_style_mixing:
                        feed_dict = {self.real_images_2: gen_img, self.w_latent_ref: w_latent_e_1,
                                     self.z_input_1: z_batch_1, self.z_input_2: z_batch_2,
                                     self.w_latent_in: w_latent_in,
                                     self.real_images_1: batch_images, self.learning_rate_input_e: self.learning_rate_e,
                                     self.style_mixing_prob: self.style_mixing,
                                     self.learning_rate_input_g: self.learning_rate_g,
                                     self.learning_rate_input_d: self.learning_rate_d}
                    ...
                    if run_epochs % print_epochs == 0 and flag_style_mixing:
                        ...
                        update_csv(model=self, file=csvs[0], variables=epoch_outputs, epoch=epoch, iteration=run_epochs,
                                   losses=losses)
                    run_epochs += 1
                ...
                if (checkpoint_every is not None and epoch % checkpoint_every == 0) or (epochs == epoch):
                    generate_samples_epoch(session=session, model=self, data=data, epoch=epoch,
                                           data_out_path=data_out_path, report=report)
                ...
                write_sprite_image(filename=os.path.join(data_out_path, 'images/gen_samples_epoch_%s.png' % epoch),
                                   data=gen_samples, metadata=False)
                ...
                gen_samples = session.run([self.output_gen], feed_dict=feed_dict)[0]
                write_sprite_image(
                    filename=os.path.join(data_out_path, 'images/gen_samples_steady_epoch_%s.png' % epoch),
                    data=gen_samples, metadata=False)
                ...
                gen_samples = session.run([self.output_gen], feed_dict=feed_dict)[0]
                write_sprite_image(
                    filename=os.path.join(data_out_path, 'images/gen_samples_recon_epoch_%s.png' % epoch),
                    data=gen_samples, metadata=False)
