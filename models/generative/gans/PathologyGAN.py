...
class PathologyGAN(GAN):
    def __init__(self,
                 ...
                 # Loss function type: Standard, Least Square, Wasserstein, Wasserstein Gradient Penalty.
    ...
    def generator(self, w_latent, reuse, is_train, init):
        output = generator_resnet_style(w_input=w_latent, image_channels=self.image_channels, layers=self.layers,
                                        spectral=self.spectral, activation=leakyReLU, reuse=reuse, is_train=is_train,
                                        normalization=self.normalization, init=init, noise_input_f=self.noise_input_f,
                                        regularizer=orthogonal_reg(self.regularizer_scale), attention=self.attention)
        return output

    # Loss function.
    def loss(self):
        loss_dis, loss_gen = losses(self.loss_type, self.output_fake, self.output_real, self.logits_fake,
                                    self.logits_real, real_images=self.real_images, fake_images=self.fake_images,
                                    top_k_samples=self.top_k_samples, discriminator=self.discriminator,
                                    gp_coeff=self.gp_coeff, init=self.init)

        return loss_dis, loss_gen

    # Optimizer.
    def optimization(self, gpus=[0]):
        train_discriminator, train_generator = optimizer(self.beta_1, self.loss_gen, self.loss_dis, self.loss_type,
                                                         self.learning_rate_input_g, self.learning_rate_input_d,
                                                         beta_2=self.beta_2, gpus=gpus)
    ...
    def style_mixing_reg2(self, w_input_1, w_input_2, style_mixing_prob, layers):
        ...
        with tf.variable_scope('style_mixing_reg'):
            ...
            w_latent = tf.cond(tf.random_uniform([], 0.0, 1.0) < style_mixing_prob, lambda: w_latent_1,
                               lambda: w_interpolation)
        return w_latent
    ...
    def build_model(self):
        ...
        with tf.device('/gpu:0'):
            ...
            self.w_latent_2 = self.mapping(self.z_input_2, reuse=True, is_train=True, normalization=None,
                                           init=self.init)
            self.w_latent = self.style_mixing_reg2(self.w_latent_1, self.w_latent_2, self.style_mixing_prob,
                                                   self.layers)
        ...
        if self.image_height > 256: i = 1
        with tf.device('/gpu:%s' % i):
            self.output_real, self.logits_real, self.feature_space_real = self.discriminator(images=self.real_images,
            ...
            for epoch in range(1, epochs + 1):
                ...
                for batch_images, batch_labels in data.training:
                    ...
                    feed_dict = {self.z_input_1: z_batch_1, self.z_input_2: z_batch_2, self.w_latent_in: w_latent_in,
                                 self.real_images: batch_images, self.style_mixing_prob: self.style_mixing,
                    ...
                    if run_epochs % print_epochs == 0:
                        epoch_loss_dis, epoch_loss_gen = session.run([self.loss_dis, self.loss_gen],
                                                                     feed_dict=feed_dict)
                        update_csv(model=self, file=csvs[0], variables=[epoch_loss_gen, epoch_loss_dis], epoch=epoch,
                                   iteration=run_epochs, losses=losses)
