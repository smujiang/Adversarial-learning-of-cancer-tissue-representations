...
def realness_loss(features_fake, features_real, anchor_0, anchor_1, v_max=1., v_min=-1., relativistic=True,
                  discriminator=None, real_images=None, fake_images=None, init=None, gp_coeff=None,
    ...
    epsilon = tf.random.uniform(shape=tf.stack([tf.shape(real_images)[0], 1, 1, 1]), minval=0.0, maxval=1.0,
...
def losses(loss_type, output_fake, output_real, logits_fake, logits_real, label=None, real_images=None,
           fake_images=None, encoder=None, discriminator=None, init=None, gp_coeff=None, hard=None,
    ...
    if 'relativistic' in loss_type:
        ...
        if 'standard' in loss_type:
            ...
            loss_dis_real = tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_diff_real_fake, labels=tf.ones_like(logits_fake)))
            ...
            if top_k_samples is not None:
                logits_diff_fake_real_top = \
                tf.math.top_k(input=tf.reshape(logits_diff_fake_real, (1, -1)), k=top_k_samples, sorted=False,
                              name='top_performance_samples_max')[0]
                logits_diff_real_fake_min = - \
                tf.math.top_k(input=tf.reshape(-logits_diff_real_fake, (1, -1)), k=top_k_samples, sorted=False,
                              name='top_performance_samples_min')[0]
                ...
                loss_gen_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_diff_fake_real_top,
                                                                                       labels=tf.ones_like(
                                                                                           logits_diff_fake_real_top)))
                loss_gen_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_diff_real_fake_min,
            ...
            if top_k_samples is not None:
                ...
                # Fake - Avg(real)
                ind_logits_diff_fake_real = tf.math.top_k(input=tf.reshape(-tf.abs(logits_diff_fake_real), (1, -1)),
                                                          k=top_k_samples, sorted=False,
                                                          name='top_performance_samples_fake_real').indices
                # For real images,
                # Real - Avg(fake) 
                ind_logits_diff_real_fake = tf.math.top_k(input=tf.reshape(-tf.abs(logits_diff_real_fake), (1, -1)),
                                                          k=top_k_samples, sorted=False,
                                                          name='top_performance_samples_real_fake').indices
                ...
                loss_gen_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=top_k_logits_fake_real,
                                                                                       labels=tf.ones_like(
                                                                                           top_k_logits_fake_real)))
                loss_gen_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=top_k_logits_real_fake,
                                                                                       labels=tf.zeros_like(
                                                                                           top_k_logits_real_fake)))
            else:
                loss_gen_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_diff_fake_real,
                                                                                       labels=tf.ones_like(
                                                                                           logits_fake)))
        ...
        loss_dis_real = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_real, labels=tf.ones_like(output_fake) * 0.9))
