...
def plot_gmm_latent_space(gmm, gmm_embedding, dataset_images, work_img_ind, img_path, field_names, file_name,
                          labels=None, title=None, radius_rate=0.8,
                          scatter_plot=True, n_cells=4, size=20, figsize=(20, 20), ellipsis=False, x_lim=None,
                          y_lim=None):
    ...
    if scatter_plot:
        ...
        if labels is not None:
            ...
            if field_names is not None:
                legend1 = main_ax.legend(scatter1.legend_elements()[0], field_names, loc="best", title=legend_title,
                                         prop={'size': size - 8})
            else:
                legend1 = main_ax.legend(*scatter1.legend_elements(), loc="best", title=legend_title,
                                         prop={'size': size - 8})
    ...
    for roi_i, m_img in enumerate(rois_img):
        ...
        if roi_i == 0:
            box_coor = [height, width]
        elif roi_i == 1 or roi_i == 2:
            box_coor = [int(height / 2), width]
        elif roi_i == 3:
            box_coor = [0, width]
        elif roi_i == 4 or roi_i == 5:
            box_coor = [0, int(height / 2)]
        elif roi_i == 6:
            box_coor = [0, 0]
        elif roi_i == 7 or roi_i == 8:
            box_coor = [int(height / 2), 0]
        elif roi_i == 9:
            box_coor = [height, 0]
        elif roi_i == 10 or roi_i == 11:
            box_coor = [width, int(height / 2)]

        axesA = f6_axes[row_index, col_index]

        color = "black"
        con = ConnectionPatch(xyA=mean, xyB=box_coor, coordsA="data", coordsB="data", axesA=main_ax, axesB=axesA,
                              color=color, arrowstyle="-", zorder=0)
        con = ConnectionPatch(xyA=mean, xyB=box_coor, coordsA="data", coordsB="data", axesA=main_ax, axesB=axesA,
                              color=color, arrowstyle="-", zorder=0)
        con = ConnectionPatch(xyA=mean, xyB=box_coor, coordsA="data", coordsB="data", axesA=main_ax, axesB=axesA,
                              color=color, arrowstyle="-", zorder=0)
...
def report_progress_latent(epoch, w_samples, img_samples, img_path, label_samples=None, storage_name='w_lat'):
    ...
    for radius in [1.51, 1.25, 1.01, 0.75, 0.61, 0.51, 0.25, 0.11, 0.01]:
        if label_samples is not None:
            ...
            labels = None
        rois, x_lim, y_lim = plot_gmm_latent_space(gmm=gmm, gmm_embedding=embedding_umap, dataset_images=img_samples,
                                                   work_img_ind=list(range(w_samples.shape[0])), img_path=img_path,
                                                   field_names=None, file_name='%s_epoch_%s_images_latent_space_%s' % (
            storage_name, epoch, str(radius).replace('.', 'p')),
                                                   labels=labels, title='', radius_rate=radius, n_cells=4, size=20,
                                                   figsize=(20, 20), scatter_plot=True, x_lim=None, y_lim=None)
    ...
    if label_samples is not None:
        ...
        for i in clusters[:10]:
            try:
                ...
                gmm.fit(class_emb)
                plot_gmm_latent_space(gmm=gmm, gmm_embedding=class_emb, dataset_images=class_img,
                                      work_img_ind=list(range(class_indxs.shape[0])), img_path=img_path,
                                      field_names=None,
                                      file_name='%s_epoch_%s_class_%s_images_latent_space_%s' % (
                                      storage_name, epoch, i, str(radius).replace('.', 'p')),
                                      labels=class_labels, title='', radius_rate=radius, n_cells=4, size=20,
                                      figsize=(20, 20), scatter_plot=True, x_lim=x_lim, y_lim=y_lim)
            except:
