...
def compute_metrics_attention(model, session, slides, patterns, latent, subset_slides=None, labels=None,
                              return_weights=False, top_perc=0.001):
    # Variables to return.
    ...
    # AUC.
    try:
        auc_per_class = roc_auc_score(y_true=model.one_hot_encoder.transform(class_set.reshape((-1, 1))),
                                      y_score=prob_set, average=None)
        ...
        for class_ in range(model.mult_class):
            try:
                fpr, tpr, thresholds = roc_curve(model.one_hot_encoder.transform(class_set.reshape((-1, 1)))[:, class_],
                                                 prob_set[:, class_])
    ...
    if not return_weights:
        ...
        return [accuracy, recall, precision, auc_all], [relevant_patches, relevant_labels, relevant_indeces,
                                                        relevant_slides, relevant_weights], [class_set, pred_set,
                                                                                             prob_set]


# Compute different metrics for given set: Accuracy, Recall, Precision, and AUC.
def compute_metrics_attention_multimag(model, session, slides, patterns, latent_20x, latent_5x, subset_slides=None,
                                       labels=None):
    # Prediction, True labels for metrics.
    prob_set = list()
    pred_set = list()
...
# Compute different metrics for given set: Accuracy, Recall, Precision, and AUC.
def compute_metrics_attention_multimagnifications(model, session, slides, patterns, latent_20x, latent_10x, latent_5x,
                                                  subset_slides=None, labels=None):
    # Prediction, True labels for metrics.
    prob_set = list()
    pred_set = list()
    class_set = list()
    ...
    for slide in unique_slides:
        ...
        # Run the model.
        feed_dict = {model.represenation_input_20x: lantents_20x_batch,
                     model.represenation_input_10x: lantents_10x_batch, model.represenation_input_5x: lantents_5x_batch}
        prob_batch = session.run([model.prob], feed_dict=feed_dict)[0]
...
# Compute different metrics for given set: Accuracy, Recall, Precision, and AUC.
def save_weights_attention(model, set_type, session, output_path, slides, patterns, latent, subset_slides=None,
                           labels=None):
    # Unique slides to iterate through.
    if subset_slides is None:
        unique_slides = list(np.unique(slides))
    ...
    with h5py.File(hdf5_path, mode='w') as hdf5_content:
        ...
        probab_storage = hdf5_content.create_dataset(name='probabilities', shape=probabilities.shape,
                                                     dtype=weights_set.dtype)
...
# Compute different metrics for given set: Accuracy, Recall, Precision, and AUC.
def save_weights_attention_multimag(model, set_type, session, output_path, slides, patterns, latent_20x, latent_5x,
                                    subset_slides=None, labels=None):
    # Unique slides to iterate through.
    if subset_slides is None:
        unique_slides = list(np.unique(slides))
    ...
    for slide in unique_slides:
        ...
        # Run the model.
        feed_dict = {model.represenation_input_20x: lantents_20x_batch, model.represenation_input_5x: lantents_5x_batch}
        prob_batch, weights_5x, weights_20x = session.run([model.prob, model.weights, model.weights_20x],
                                                          feed_dict=feed_dict)
    ...
    hdf5_path = os.path.join(output_path, 'hdf5_attention_weights_%s.h5' % set_type)
    with h5py.File(hdf5_path, mode='w') as hdf5_content:
        weight_20x_storage = hdf5_content.create_dataset(name='weights_20x', shape=weights_20x_set.shape,
                                                         dtype=weights_20x_set.dtype)
        weight_5x_storage = hdf5_content.create_dataset(name='weights_5x', shape=weights_5x_set.shape,
                                                        dtype=weights_5x_set.dtype)
        probab_storage = hdf5_content.create_dataset(name='probabilities', shape=probabilities.shape,
                                                     dtype=weights_5x_set.dtype)
...
# Compute different metrics for given set: Accuracy, Recall, Precision, and AUC.
def save_weights_attention_multimagnifications(model, set_type, session, output_path, slides, patterns, latent_20x,
                                               latent_10x, latent_5x, subset_slides=None, labels=None):
    # Unique slides to iterate through.
    if subset_slides is None:
        unique_slides = list(np.unique(slides))
    # Use specified slides: Histopology subtypes.
    ...
    for slide in unique_slides:
        ...
        feed_dict = {model.represenation_input_20x: lantents_20x_batch,
                     model.represenation_input_10x: lantents_10x_batch, model.represenation_input_5x: lantents_5x_batch}
        prob_batch, weights_5x, weights_10x, weights_20x = session.run(
            [model.prob, model.weights, model.weights_10x, model.weights_20x], feed_dict=feed_dict)
    ...
    with h5py.File(hdf5_path, mode='w') as hdf5_content:
        weight_20x_storage = hdf5_content.create_dataset(name='weights_20x', shape=weights_20x_set.shape,
                                                         dtype=weights_20x_set.dtype)
        weight_10x_storage = hdf5_content.create_dataset(name='weights_10x', shape=weights_10x_set.shape,
                                                         dtype=weights_10x_set.dtype)
        weight_5x_storage = hdf5_content.create_dataset(name='weights_5x', shape=weights_5x_set.shape,
                                                        dtype=weights_5x_set.dtype)
        probab_storage = hdf5_content.create_dataset(name='probabilities', shape=probabilities.shape,
                                                     dtype=weights_5x_set.dtype)
