...
def generate_samples_epoch(session, model, data, epoch, data_out_path, num_samples=10000, batch_size=50,
                           one_hot_encoder=None, report=False):
    ...
    img_storage = hdf5_file.create_dataset(name='images',
                                           shape=[num_samples, data.patch_h, data.patch_w, data.n_channels],
                                           dtype=np.float32)
    ...
    while ind < num_samples:
        ...
        if one_hot_encoder is not None:
            ...
            w_latent_batch, l_embedding_batch = session.run([model.w_latent_out, model.label_emb_gen],
                                                            feed_dict=feed_dict)
        else:
    ...
    if report:
        ...
        try:
            report_progress_latent(epoch=epoch, w_samples=w_storage, img_samples=img_storage,
                                   img_path=hdf5_path.split('/hdf5')[0], label_samples=label_samples)
        except:
...
# Generate sampeles from PathologyGAN, no encoder.
def generate_samples_from_checkpoint_conditional(model, data, data_out_path, checkpoint, k, num_samples=5000,
                                                 batches=50, save_img=False):
    path = os.path.join(data_out_path, 'results')
    path = os.path.join(path, model.model_name)
    path = os.path.join(path, data.dataset)
    path = os.path.join(path, data.marker)
    res = 'h%s_w%s_n%s_zdim%s_emb%s_nclust%s' % (
    data.patch_h, data.patch_w, data.n_channels, model.z_dim, model.embedding_size, k)
    path = os.path.join(path, res)
    img_path = os.path.join(path, 'generated_images')
    if not os.path.isdir(path):
        os.makedirs(path)
...
def discriminator_features_from_checkpoint(model, data, data_out_path, checkpoint, real_hdf5, clust_percent=1.0,
                                           clusters_num=None, batches=50, save_img=False):
    os.umask(0o002)
    path = os.path.join(data_out_path, 'results')
    path = os.path.join(path, model.model_name)
    path = os.path.join(path, data.dataset)
        ...
        with h5py.File(real_hdf5, mode='r') as hdf5_file:
            with h5py.File(hdf5_path, mode='w') as hdf5_file_w:
                ...
                for key in hdf5_file.keys():
                    ...
                    if 'image' in key or 'img' in key:
                        if save_img: img_storage = hdf5_file_w.create_dataset(name='images', shape=key_shape,
                                                                              dtype=np.float32)
                        feature_storage = hdf5_file_w.create_dataset(name='features', shape=[num_samples] + [
                            model.feature_space_real.shape[1]], dtype=np.float32)
                        ...
                        with tf.Session() as session:
                            ...
                            print('Number of Real Images:', num_samples, 'Feature space size:',
                                  model.feature_space_real.shape[1])
                            print('Starting encoding...')
                        ...
                        if clusters_num is not None:
                            ...
                            feature_labels_storage = hdf5_file_w.create_dataset(name='feat_cluster_labels',
                                                                                shape=[num_samples] + [1],
                                                                                dtype=np.float32)
                            initialization = 'k-means++'
                            kmeans = KMeans(init=initialization, n_clusters=clusters_num, n_init=10).fit(
...
def real_encode_contrastive_from_checkpoint(model, data, data_out_path, checkpoint, real_hdf5, batches=50,
                                            save_img=False):
    # Directory handling.
    path = os.path.join(data_out_path, 'results')
    path = os.path.join(path, model.model_name)
            ...
            with h5py.File(hdf5_path, mode='w') as hdf5_file_w:
                ...
                for key in hdf5_file.keys():
                    ...
                    if 'image' in key or 'img' in key:
                        ...
                        h_storage = hdf5_file_w.create_dataset(name='%s_h_latent' % key, shape=h_latent_shape,
                                                               dtype=np.float32)
                        z_storage = hdf5_file_w.create_dataset(name='%s_z_latent' % key, shape=z_latent_shape,
                                                               dtype=np.float32)
                        if save_img: img_storage = hdf5_file_w.create_dataset(name='%s' % key, shape=key_shape,
                                                                              dtype=np.float32)
                        ...
                        with tf.Session() as session:
                            ...
                            while ind < num_samples:
                                ...
                                # Encode real images into W latent space.
                                feed_dict = {model.real_images_2: real_img_batch}
                                h_rep_batch, z_rep_batch = session.run([model.h_rep_out, model.z_rep_out],
                                                                       feed_dict=feed_dict)
